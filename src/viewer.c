...
static viewer *current_viewer = NULL;         // For signal handler (resizing window)
...
viewer *
viewer_alloc(char **data, size_t data_n)
{
        viewer *m = (viewer*)malloc(sizeof(viewer));
...
viewer_display(viewer *m)
{
        ...
        while (1) {
                ...
                if (resize_flag) {
                        viewer_dump(m); // Immediate redraw after resize
                        resize_flag = 0;
                }
        }
 done:
